...
function McpSection() {
  ...
  return (
    <section className="mt-8">
      ...
      <p className="mb-3 text-[12px] leading-relaxed text-muted">
        Optional, for developers: Claude Code and Claude Desktop can drive
        Hangar directly — create projects, file things, complete tasks. Local
        only (127.0.0.1), bearer-token protected, and entirely off unless you
        want it.
      </p>
      <label className="mb-3 flex w-fit cursor-pointer items-center gap-2.5 text-[12.5px]">
        <input
          type="checkbox"
          checked={enabled ?? true}
          onChange={async (e) => {
            await api.mcpSetEnabled(e.target.checked);
            push(
              e.target.checked
                ? "MCP server will start next launch"
                : "MCP server disabled — takes effect next launch",
            );
            qc.invalidateQueries({ queryKey: ["mcpEnabled"] });
          }}
        />
        Run the MCP server when Hangar starts
      </label>
      {enabled !== false && info && (
